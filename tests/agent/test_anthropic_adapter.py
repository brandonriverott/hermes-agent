...
class TestRunOauthSetupToken:
    ...
    def test_returns_token_from_credential_files(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr("agent.anthropic_adapter.Path.home", lambda: tmp_path)
        # macOS-only: read_claude_code_credentials() also probes the Keychain via
        # subprocess.run, whose globally-patched .stdout is a MagicMock -> json.loads
        # TypeError. Neutralize it so the file/env path under test is what runs.
    ...
    def test_returns_token_from_env_var(self, monkeypatch, tmp_path):
        ...
        monkeypatch.delenv("ANTHROPIC_TOKEN", raising=False)
        monkeypatch.setattr("agent.anthropic_adapter.Path.home", lambda: tmp_path)
        # macOS-only Keychain probe shares the patched subprocess.run mock; neutralize it.
        monkeypatch.setattr("agent.anthropic_adapter._read_claude_code_credentials_from_keychain", lambda: None)
